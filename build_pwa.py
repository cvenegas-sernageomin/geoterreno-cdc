...
async function renderPunto(app){
  ...
  {
    const fmCard=el('div',{class:'card'});
    const raices=await fotomapasRaiz(curPunto.id);
    fmCard.append(el('h2',{},'🖼️ Foto-mapa — cambio de escala ('+raices.length+')'));
    fmCard.append(el('div',{class:'muted',html:'Usa una foto como lienzo y coloca sobre ella fotos de detalle; cada detalle es a su vez otro lienzo (multiescala).'}));
    ...
    fmCard.append(box);
    fmCard.append(el('div',{class:'btnbar'},
      el('button',{class:'btn mini',onclick:async()=>{await guardarPunto();const reg={id:uid(),_parent:null,_punto:curPunto.id,_pins:null};await put('fotomapa',reg);editorFotomapa(reg);}},'+ Nuevo foto-mapa')));
    app.append(fmCard);
  }
...
const CACHE='geoterreno-cdc-v17';
